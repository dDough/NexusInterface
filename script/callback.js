...
AJAX.LOADER.Show = function(LoaderID) {
	...
	document.getElementById(LoaderID).style.opacity  = 1;
};
...
AJAX.LOADER.Hide = function(LoaderID) {
	...
	/** Set the Loader Opacity. **/
	document.getElementById(LoaderID).style.opacity  = 0;
};
...
AJAX.CALLBACK.Execute = function(ResponseObject, TagID) {
	...
	/** Displace space with 404 Error if Response Status Code is not 200. **/
		...
		document.getElementById(TagID).innerHTML = "<img src='images/404.png' class='center-screen'>";
...
AJAX.CALLBACK.Final = function(ResponseObject, TagID) {
	...
	if(TagID !== undefined && TagID !== '') {
		...
		setTimeout(function() { document.getElementById(TagID).classList.remove("fade-transition"); }, 300);
	}
};
...
AJAX.CALLBACK.Standard = function(ResponseObject, Address, TagID) {
	...
	if(AJAX.CALLBACK.Init(ResponseObject, TagID))
		return;
	...
	setTimeout(function() { AJAX.CALLBACK.Final(ResponseObject, TagID); },   250);
};
